...
static int __init s5pv210_cpufreq_driver_init(struct cpufreq_policy *policy)
{
	...
	policy->cpuinfo.transition_latency = 30000; /* 30 us */
...
//Set initial max speed to 1ghz for people who don't want to overclock
	policy->max = 1000000;
	policy->min = 100000;
#ifdef CONFIG_S5PV210_CPU_FREQ_SET_MIN_MAX
        policy->min = CONFIG_S5PV210_CPU_FREQ_MIN;
        policy->max = CONFIG_S5PV210_CPU_FREQ_MAX;
#endif
	return 0;
}
